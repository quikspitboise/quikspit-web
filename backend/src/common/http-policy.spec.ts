...
describe('HTTPS redirects and proxy trust', () => {
  ...
  beforeEach(() => {
    process.env = { ENFORCE_HTTPS: 'true', CANONICAL_API_ORIGIN: 'https://api.example.test' };
  });
  ...
  it('ignores a forged Host and untrusted forwarded protocol', async () => {
    await request(app()).post('/check?x=1').set('Host', 'attacker.test')
      .set('X-Forwarded-Proto', 'https').expect(308)
      .expect('Location', 'https://api.example.test/check?x=1');
    await request(app()).get('//attacker.test/path').expect(308)
  ...
  it('uses forwarded client addresses only from configured ingress peers', async () => {
    ...
    await request(app()).get('/check').set('X-Forwarded-Proto', 'https')
      .set('X-Forwarded-For', '198.51.100.7').expect(200).expect({ ip: '198.51.100.7' });
    process.env.TRUST_PROXY = '10.0.0.0/8';
    await request(app()).get('/check').set('X-Forwarded-Proto', 'https').expect(308);
