...
export function configureProxyTrust(app: Express): void {
  const proxies = (process.env.TRUST_PROXY || '').split(',').map((value) => value.trim()).filter(Boolean);
  ...
  if (proxies.some((value) => /^(true|false|\d+)$/i.test(value))) {
    throw new Error('TRUST_PROXY must contain trusted proxy IP addresses or CIDR ranges');
  }
...
export function canonicalHttpsRedirect() {
  ...
  if (origin.protocol !== 'https:' || origin.username || origin.password ||
    ...
    throw new Error('CANONICAL_API_ORIGIN must be an HTTPS origin without a path or credentials');
  }
