...
export class InvoiceService {
  ...
  constructor(
    ...
    if (!process.env.STRIPE_SECRET_KEY) {
      this.logger.warn(
        'STRIPE_SECRET_KEY is not configured - Invoice service will not be available',
      );
      ...
      try {
        ...
        this.logger.error(
          'Failed to initialize Stripe for invoices',
  ...
  async sendInvoice(dto: SendInvoiceDto): Promise<InvoiceResult> {
    ...
    if (sendViaSms && !dto.customerPhone) {
      throw new BadRequestException(
        'customerPhone is required when sendViaSms is true',
      );
    }
    ...
    try {
      ...
      return {
        ...
        customerId:
          typeof finalInvoice.customer === 'string'
