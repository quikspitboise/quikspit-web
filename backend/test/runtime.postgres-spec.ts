...
describe('shared runtime state with PostgreSQL', () => {
  ...
  beforeAll(async () => { context = await createPostgresTestContext(); });
  ...
  it('admits exactly the configured number of concurrent requests across instances', async () => {
    ...
    const results = await Promise.all(Array.from({ length: 12 }, (_, index) =>
      (index % 2 ? first : second).increment(key, 60_000, 3, 60_000, 'default')));
    expect(results.filter((result) => !result.isBlocked)).toHaveLength(3);
    expect(results.filter((result) => result.isBlocked)).toHaveLength(9);
    expect((await first.increment(randomUUID(), 60_000, 3, 60_000, 'default')).isBlocked).toBe(false);
    await context.dataSource.query(
      "UPDATE request_limits SET expires_at = now() - interval '1 second', blocked_until = now() - interval '1 second' WHERE key = $1", [key],
    );
    expect(await second.increment(key, 60_000, 3, 60_000, 'default')).toMatchObject({ totalHits: 1, isBlocked: false });
  });
  ...
  it('shares successful reviews and backs off after a failed refresh without losing stale data', async () => {
    ...
    expect(await second.read(key)).toMatchObject({ value: { rating: 4.8 }, fresh: true });
    await context.dataSource.query(
      "UPDATE provider_cache SET expires_at = now() - interval '1 second', refresh_after = now() - interval '1 second' WHERE key = $1", [key],
    );
    const retryLease = await second.claim(key);
    await second.fail(key, retryLease!);
    expect(await first.claim(key)).toBeNull();
    expect(await first.read(key)).toMatchObject({ value: { rating: 4.8 }, fresh: false });
  });
  ...
  it('recovers an abandoned lease and rejects writes by the expired owner', async () => {
    ...
    await context.dataSource.query(
      "UPDATE provider_cache SET lease_until = now() - interval '1 second' WHERE key = $1", [key],
    );
  ...
  it('can roll back and replay every registered migration in an isolated schema', async () => {
    ...
    try {
      const migrations = await isolated.dataSource.query(`SELECT * FROM "${isolated.schema}".migrations`);
      for (const _migration of migrations) await isolated.dataSource.undoLastMigration();
      await isolated.dataSource.runMigrations({ transaction: 'all' });
      expect((await isolated.dataSource.query(`SELECT * FROM "${isolated.schema}".migrations`)).length).toBe(migrations.length);
    } finally {
